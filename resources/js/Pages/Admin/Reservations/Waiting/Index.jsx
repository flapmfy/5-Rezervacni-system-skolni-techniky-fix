...
const Waiting = ({ waitingReservations, filters }) => {
  const route = useRoute();
  const [search, setSearch] = useState(filters.vyhledavani || '');

  const updateFilters = (newFilters) => {
    router.get(
      route('admin.reservations.waiting'),
      { ...filters, ...newFilters },
      {
        preserveState: true,
        replace: true,
        preserveScroll: true,
        only: ['waitingReservations', 'filters'],
      }
    );
  };
  ...
  return (
    <>
      ...
      <div className="container pb-2">
        <h1 className="fluid-text-4 font-bold">Neschválené žádosti</h1>
        ...
        {/* vypis rezervaci */}
        {!waitingReservations.data.length ? (
          ...
          <>
            <div className="mt-4 overflow-x-auto rounded-lg border shadow dark:border-gray-700">
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                  {waitingReservations.data.map((reservation, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="whitespace-nowrap px-6 py-4 text-sm">
                          {(waitingReservations.current_page - 1) * waitingReservations.per_page +
                            index +
                            1}
                        </td>
            ...
            <div className="mt-8 flex justify-center">
              <Pagination links={waitingReservations.links} />
            </div>
          </>
        )}
      </div>
